...
“VCF 质量评估”页签按文件、位点、样本、样本对和群体五层生成本地深度质量报告。CallVCF现在聚焦植物分析，内置通用二倍体植物、自交植物/纯系、通用多倍体植物、棉花群体、棉花自交系和植物单倍体/细胞器 Profile。动物不再提供任何内置默认Profile：必须进入“自定义物种与阈值”，明确填写物种、GT编码倍性、繁殖方式及12项核心阈值，并在运行前确认参数解释责任；前后端都会拒绝绕过该约束。
